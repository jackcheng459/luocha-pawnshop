...
- 新增命主故事卡，作为命格卡、当票卡之外的第三张可导出 PNG 分享卡。
- 离店界面在命主故事生成后显示“导出命主故事卡”。
- `/story/{storyId}` 故事页新增“保存这个故事”导出按钮。
- 新增本地夜账索引 `luocha:yezhang`，仅保存 storyId、命格名、判词片段、时间和 visitedAt。
- 首页仅在本机存在夜账记录时显示隐藏入口，夜账弹层支持翻看旧夜和二次确认清空。
- 故事页本身不写入夜账，朋友扫码只读故事，不会进入访问者夜账。
